 ...
 *
 * In remote mode, `<ai-agent>` strips handler functions from `AiRequestOptions.tools`
...
export function registerTool(name: string, handler: AiToolHandler): void {
  if (!name) throw new Error("[@csbc-dev/ai-agent] registerTool: name is required.");
  if (typeof handler !== "function") {
    throw new Error("[@csbc-dev/ai-agent] registerTool: handler must be a function.");
  }

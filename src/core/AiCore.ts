...
export class AiCore extends EventTarget {
  ...
  private _setContent(content: string): void {
    ...
    this._target.dispatchEvent(new CustomEvent("ai-agent:content-changed", {
      detail: content,
      bubbles: true,
    }));
  }
  ...
  private _setUsage(usage: AiUsage | null): void {
    this._usage = usage;
    this._target.dispatchEvent(new CustomEvent("ai-agent:usage-changed", {
      detail: usage,
      bubbles: true,
    }));
  }
  ...
  private _setLoading(loading: boolean): void {
    ...
    this._target.dispatchEvent(new CustomEvent("ai-agent:loading-changed", {
      detail: loading,
      bubbles: true,
    }));
  }
  ...
  private _setStreaming(streaming: boolean): void {
    ...
    this._target.dispatchEvent(new CustomEvent("ai-agent:streaming-changed", {
      detail: streaming,
      bubbles: true,
    }));
  }
  ...
  send(prompt: string | AiContentPart[], options: AiRequestOptions): Promise<string | null> {
    ...
    if (!options.model) raiseError("model is required. See @csbc-dev/ai-agent README §Supported Providers for each provider's model catalog (no default is shipped because model identifiers drift faster than library releases).");
    ...
    try {
      while (true) {
        ...
        if (roundtrips > maxRoundtrips) {
          throw new Error(
            `[@csbc-dev/ai-agent] maxToolRoundtrips (${maxRoundtrips}) exceeded; aborting tool-use loop.`,
          );
        }
        ...
        const toolResults = await Promise.all(turn.toolCalls!.map(async (call) => {
          this._target.dispatchEvent(new CustomEvent("ai-agent:tool-call-requested", {

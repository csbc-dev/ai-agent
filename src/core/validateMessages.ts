 ...
 * setter, the `<ai-agent>` `messages` property, or the remote `setWithAck`
...
export function validateMessages(messages: AiMessage[]): void {
  if (!Array.isArray(messages)) {
    throw new Error("[@csbc-dev/ai-agent] messages must be an array of AiMessage.");
  }
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (!m || typeof m !== "object") {
      throw new Error(`[@csbc-dev/ai-agent] messages[${i}] must be an object.`);
    }
    if (!VALID_ROLES.has(m.role)) {
      throw new Error(`[@csbc-dev/ai-agent] messages[${i}].role must be one of "system", "user", "assistant", "tool"; got "${m.role}".`);
    }
    if (typeof m.content !== "string" && !Array.isArray(m.content)) {
      throw new Error(`[@csbc-dev/ai-agent] messages[${i}].content must be a string or AiContentPart[].`);
    }
    ...
    if (m.role === "tool") {
      if (typeof m.toolCallId !== "string" || m.toolCallId === "") {
        throw new Error(`[@csbc-dev/ai-agent] messages[${i}] with role "tool" requires a non-empty string toolCallId.`);
      }
      if (!pendingToolCallIds.has(m.toolCallId)) {
        if (seenToolCallIds.has(m.toolCallId)) {
          throw new Error(`[@csbc-dev/ai-agent] messages[${i}] with role "tool" replays toolCallId "${m.toolCallId}" that was already consumed by an earlier tool message. Each tool call correlates to exactly one tool response.`);
        }
        throw new Error(`[@csbc-dev/ai-agent] messages[${i}] with role "tool" references toolCallId "${m.toolCallId}" that does not correlate to any prior assistant tool call. Truncated or orphaned tool results break provider wire format.`);
      }
    ...
    if (m.role === "assistant" && m.toolCalls !== undefined) {
      if (!Array.isArray(m.toolCalls)) {
        throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls must be an array when present.`);
      }
      for (let j = 0; j < m.toolCalls.length; j++) {
        const tc = m.toolCalls[j];
        if (!tc || typeof tc !== "object") {
          throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls[${j}] must be an object.`);
        }
        if (typeof tc.id !== "string" || tc.id === "") {
          throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls[${j}].id must be a non-empty string.`);
        }
        if (typeof tc.name !== "string" || tc.name === "") {
          throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls[${j}].name must be a non-empty string.`);
        }
        if (typeof tc.arguments !== "string") {
          throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls[${j}].arguments must be a string (JSON-encoded arguments).`);
        }
        if (seenToolCallIds.has(tc.id)) {
          throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls[${j}].id "${tc.id}" duplicates an id already declared earlier. Tool call ids must be unique across the conversation.`);
        }
    ...
    if (m.role !== "assistant" && (m as any).toolCalls !== undefined) {
      throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCalls is only valid on assistant messages; got role "${m.role}".`);
    }
    if (m.role !== "tool" && (m as any).toolCallId !== undefined) {
      throw new Error(`[@csbc-dev/ai-agent] messages[${i}].toolCallId is only valid on tool messages; got role "${m.role}".`);
    }
  }
}

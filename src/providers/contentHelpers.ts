  ...
  for (const p of parts) {
    if (p.type === "text") {
      if (typeof p.text !== "string") {
        throw new Error("[@csbc-dev/ai-agent] content part of type 'text' requires a string `text` field.");
      }
    } else if (p.type === "image") {
      if (typeof p.url !== "string" || p.url === "") {
        throw new Error("[@csbc-dev/ai-agent] content part of type 'image' requires a non-empty `url` field.");
      }
    } else {
      throw new Error(`[@csbc-dev/ai-agent] unknown content part type: ${(p as any).type}. Only 'text' and 'image' are supported in v1.`);
    }
  }
}

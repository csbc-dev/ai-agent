...
このリポジトリ (`@csbc-dev/ai-agent`) は [`@wc-bindable/ai`](https://github.com/wc-bindable-protocol/wc-bindable-protocol/tree/main/packages/ai) を起点として、csbc-dev/arch のアーキテクチャ群の一員として再パッケージしたものです。設計思想を理解するうえで前提となる 2 つのドキュメントを以下にまとめます。
...
`@csbc-dev/ai-agent` は **ケース B1**: AI 推論の決定（プロバイダ選択、ストリーミング、ツール呼び出しループ、会話状態）はすべて `AiCore` (Core, `EventTarget`) が持ち、`<ai-agent>` (Shell, `HTMLElement`) はバインダブル状態のローカル公開とコマンド転送だけを担う。リモートモードでは Core がサーバ側に置かれ、API キーや認可がクライアントに漏れない構成になる。

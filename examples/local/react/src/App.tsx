...
export default function App() {
  ...
  // snapshot of its wcBindable properties. `content` updates on every
  // streaming chunk (rAF-batched by AiCore).
  ...
  function send() {
    ...
    // inference. Failures also surface through the `error` property
    // (rendered below), so the rejection only needs to be swallowed.
    el.prompt = text;
    el.send().catch(() => {});
  }
  ...
  return (
    <div className="app">
      <div className="hero">
        <h1>React — @csbc-dev/ai-agent</h1>
        <p>
          <code>useWcBindable</code> from <code>@wc-bindable/react</code> turns{" "}
          <code>&lt;ai-agent&gt;</code> state into a React hook.
        </p>
      </div>

...
// Identical to the local-mode example — useWcBindable does not care whether
// the Core behind <ai-agent> is local or proxied over WebSocket.
...
function send() {
  ...
  // through `values.error` (rendered below).
  el.send().catch(() => {});
}
...
<template>
  <div class="app">
    <div class="hero">
      <h1>Vue — @csbc-dev/ai-agent (remote)</h1>
      <p>
    ...
    <ai-agent :ref="binding.ref" :provider="provider" :model="model" />

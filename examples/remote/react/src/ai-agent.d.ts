...
type AiAgentAttrs = DetailedHTMLProps<HTMLAttributes<AiAgent>, AiAgent> & {
  provider?: string;
  model?: string;
  // base-url / api-key are intentionally unused in remote mode — the server
  // owns provider credentials — but kept here so the type stays general.
  "base-url"?: string;
  "api-key"?: string;
  system?: string;
  ...
  "forward-credentials"?: string | boolean;
};

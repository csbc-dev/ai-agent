...
describe("AiCore", () => {
  ...
  describe("provider", () => {
    ...
    it("不明なプロバイダ名でエラーをスローする", () => {
      const core = new AiCore();
      expect(() => { core.provider = "unknown"; }).toThrow('[@csbc-dev/ai-agent] Unknown provider');
    });
  });
  ...
  describe("messages", () => {
    ...
    it("設定時にイベントが発火する", () => {
      const core = new AiCore();
      const events: any[] = [];
      core.addEventListener("ai-agent:messages-changed", (e: Event) => {
  ...
  describe("send (非ストリーミング)", () => {
    it("prompt未指定時にエラーをスローする", () => {
      const core = new AiCore();
      core.provider = "openai";
      expect(() => core.send("", { model: "gpt-4o" })).toThrow("[@csbc-dev/ai-agent] prompt is required.");
    });

    it("provider未設定時にエラーをスローする", () => {
      const core = new AiCore();
      expect(() => core.send("Hello", { model: "gpt-4o" })).toThrow("[@csbc-dev/ai-agent] provider is required.");
    });
    ...
    it("model未指定時にエラーをスローする", () => {
      ...
      expect(() => core.send("Hello", { model: "" })).toThrow(
        "[@csbc-dev/ai-agent] model is required. See @csbc-dev/ai-agent README §Supported Providers for each provider's model catalog (no default is shipped because model identifiers drift faster than library releases).",
      );
    });
    ...
    it("新しいリクエスト開始時にusage-changedイベントでnullが通知される", async () => {
      ...
      core.addEventListener("ai-agent:usage-changed", (e: Event) => {
  ...
  describe("send (ストリーミング)", () => {
    ...
    it("ストリーミング中にloading/streamingイベントが発火する", async () => {
      ...
      core.addEventListener("ai-agent:loading-changed", (e: Event) => {
        events.push({ name: "loading", detail: (e as CustomEvent).detail });
      });
      core.addEventListener("ai-agent:streaming-changed", (e: Event) => {
  ...
  describe("target指定", () => {
    it("target未指定時はイベントが自身にディスパッチされる", async () => {
      ...
      core.provider = "openai";
      const events: string[] = [];
      core.addEventListener("ai-agent:content-changed", () => events.push("content"));
    ...
    it("target指定時はイベントがtargetにディスパッチされる", async () => {
      ...
      core.addEventListener("ai-agent:content-changed", () => coreEvents.push("content"));
      target.addEventListener("ai-agent:content-changed", () => targetEvents.push("content"));
  ...
  describe("tool use (auto-loop)", () => {
    ...
    it("tool-call-requested / tool-call-completed イベントが発火する", async () => {
      ...
      const completed: any[] = [];
      const core = new AiCore();
      core.addEventListener("ai-agent:tool-call-requested", (e) => requested.push((e as CustomEvent).detail));
      core.addEventListener("ai-agent:tool-call-completed", (e) => completed.push((e as CustomEvent).detail));
      core.provider = "openai";
    ...
    it("tool-call-completedイベントは capability-boundary と handler 不在を区別可能なerrorを載せる", async () => {
      ...
      coreA.addEventListener("ai-agent:tool-call-completed", e => eventsA.push((e as CustomEvent).detail));
      ...
      coreB.addEventListener("ai-agent:tool-call-completed", e => eventsB.push((e as CustomEvent).detail));
    ...
    it("handlerが無くてもregisterToolされていればregistry経由で実行される（remote模倣）", async () => {
      ...
      const result = await core.send("go", {
        model: "gpt-4o",
        stream: false,
        // handler abstractly stripped (as <ai-agent> does in remote mode).
        tools: [{
